...
const BACKUP_APP_NAME = "Fio & Açucar";
const BACKUP_SCHEMA = "fio-acucar-inventory-backup";
const BACKUP_VERSION = 1;
const BACKUP_FILE_PREFIX = "backup_estoque_fio_acucar";
...
function bindDomReferences() {
  ...
  dom.pieceQuantityInput = document.querySelector("#pieceQuantityInput");
...
function bindEvents() {
  ...
  dom.pieceQuantityInput.addEventListener("input", updateBudgetPieceQuantity);
...
function normalizeBudget(budget) {
  ...
  return {
    ...
    pieceQuantity: normalizePieceQuantity(budget.pieceQuantity ?? budget.quantidadePecas ?? budget.sessionDuration ?? budget.quantity),
    profitMarginValue,
...
function renderBudget() {
  ...
  dom.pieceQuantityInput.value = activeBudget.pieceQuantity > 0 ? formatEditableNumber(activeBudget.pieceQuantity) : "1";
...
function updateBudgetPieceQuantity() {
  const activeBudget = getActiveBudget();

  updateAppStateWithoutFullRender(() => {
    activeBudget.pieceQuantity = normalizePieceQuantity(dom.pieceQuantityInput.value);
  });
  scheduleBudgetInputRefresh();
}
...
function updateBudgetDiscount() {
  ...
  updateAppStateWithoutFullRender(() => {
    ...
    activeBudget.discountType = ADJUSTMENT_PERCENT;
...
function renderMobileBudgetSummary(activeBudget, totals) {
  ...
  const pieceQuantity = normalizePieceQuantity(activeBudget.pieceQuantity);
  dom.mobileBudgetSummaryCount.textContent = `${formatNumber(pieceQuantity)} ${pieceQuantity === 1 ? "peça" : "peças"}`;
}
...
function isGeneratedBudget(budget) {
  ...
  return Boolean(sanitizeText(budget.clientName))
    ...
    || normalizePieceQuantity(budget.pieceQuantity) !== DEFAULT_BUDGET.pieceQuantity
...
function normalizePieceQuantity(value) {
  return Math.max(Math.round(normalizeNumber(value)), 1);
}
...
function sanitizeFileName(value) {
  return normalizeSearch(value)
    .replace(/[^a-z0-9]+/g, "-")
    .replace(/^-|-$/g, "")
    .slice(0, 80) || "orcamento-fio-acucar";
}
...
async function exportPdf() {
  const activeBudget = getActiveBudget();
  const fileName = `${sanitizeFileName(activeBudget.name || "orcamento-fio-acucar")}.pdf`;
  const totals = calculateBudgetTotals(activeBudget);
  const pieceQuantity = normalizePieceQuantity(activeBudget.pieceQuantity);
